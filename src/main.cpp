...
int main(int argc, char *argv[]) {
  ...
  if (ft17_input_file_names.size() == 0) {
    std::printf("Error! Please specify input file names for ft17( -f ).\r\n");
    exit(1);
  }
  if (teradyne_input_file_names.size() == 0) {
    std::printf("Error! Please specify input file names for teradyne( -t ).\r\n");
    exit(1);
  }
  if (output_file_name.length() == 0) {
    std::printf("Error! Please specify output fie name.\r\n");
    exit(1);
  }
  ...
  std::string table_header(
      "Lot;JobName;TeradyneTest#;FT17Test#;TeradynePin;FT17Pin;TeradyneMeasured;Unit;FT17Measured;Unit;Diff;Unit\r\n");
    ...
    for (unsigned int j = 0; j < _ft17_parser->get_metadata()->size(); j++) {
      ...
      if ((std::get<0>(_teradyne_parser->get_metadata()->at(i)) == std::get<0>(_ft17_parser->get_metadata()->at(j))) &&
        ...
        std::pair<std::map<std::string, std::string>, std::map<int, std::map<std::string, std::string>>>
            *teradyne_lot_metadata = &(std::get<2>(_teradyne_parser->get_metadata()->at(i)));
        std::pair<std::map<std::string, std::string>, std::map<int, std::map<std::string, std::string>>>
            *ft17_lot_metadata = &(std::get<2>(_ft17_parser->get_metadata()->at(j)));
        ...
        for (auto teradyne_lot_tests_meta_it = teradyne_lot_tests_meta->begin();
             teradyne_lot_tests_meta_it != teradyne_lot_tests_meta->end(); teradyne_lot_tests_meta_it++) {

          for (auto ft17_lot_tests_meta_it = ft17_lot_tests_meta->begin();
               ft17_lot_tests_meta_it != ft17_lot_tests_meta->end(); ft17_lot_tests_meta_it++) {
            ...
            if ((teradyne_test_name == ft17_test_name) && (teradyne_test_pin == ft17_test_pin)) {

              std::map<std::string, int> units = {{"pA", -12}, {"nA", -9}, {"uA", -6},  {"mA", -3}, {"A", 0},
                                                  {"pV", -12}, {"nV", -9}, {"uV", -6},  {"mV", -3}, {"V", 0},
                                                  {"пА", -12}, {"нА", -9}, {"мкА", -6}, {"мА", -3}, {"А", 0},
                                                  {"пВ", -12}, {"нВ", -9}, {"мкВ", -6}, {"мВ", -3}, {"В", 0}};
              ...
              double teradyne_measured =
                  std::atof(QString::fromStdString(teradyne_lot_tests_meta_it->second.at("test_measured"))
                                .split(" ")[0]
                                .toStdString()
                                .c_str());
              std::string teradyne_unit = QString::fromStdString(teradyne_lot_tests_meta_it->second.at("test_measured"))
                                              .split(" ")[1]
                                              .toStdString();
              ...
              std::string table_str = ";;" + teradyne_test_number + ";" + ft17_test_number + ";" + teradyne_test_pin +
                                      ";" + ft17_test_pin + ";" + std::to_string(teradyne_measured) + ";" +
                                      teradyne_unit + ";" + std::to_string(ft17_measured) + ";" + ft17_unit + ";" +
                                      std::to_string(diff) + ";" + diff_unit + "\r\n";

              if (job_name == "BEFORE_ETT")
                before_ett_table.push_back(table_str);
              else if (job_name == "AFTER_ETT")
                after_ett_table.push_back(table_str);
              else if (job_name == "PLUS")
                plus_table.push_back(table_str);
              else if (job_name == "MINUS")
                minus_table.push_back(table_str);
  ...
  if (output_stream.is_open()) {
    ...
    for (unsigned int i = 0; i < table.size(); i++)
      output_stream << table[i];

  } else
    std::printf("Failed to open \"%s\" file\r\n", output_file_name.c_str());
