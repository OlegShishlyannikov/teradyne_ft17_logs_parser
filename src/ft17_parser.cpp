...
ft17_parser::ft17_parser( std::string & ft17_raw_data )
{
  ...
  for( std::string block : blocks ){
	...
	if( job_name == "ЭТТдо" ) job_name = "BEFORE_ETT";
	else if( job_name == "ЭТТпосле" ) job_name = "AFTER_ETT";
	else if( job_name == "Т-" ) job_name = "MINUS";
	else if( job_name == "Т+" ) job_name = "PLUS";
	...
	std::tuple< int, std::string, std::pair< std::map< std::string, std::string >, std::map< int, std::map< std::string, std::string >>>, std::string, std::string >
	  lot_meta = { lot, job_name, std::make_pair( header_metadata, tests_metadata ), std::get< 0 >( tests_result ), std::get< 1 >( tests_result )};

	bool exists = false;
	int pos = 0;
	for( unsigned int i = 0; i < ft17_metadata.size(); i ++ ){

	  if(( std::get< 0 >( lot_meta ) == std::get< 0 >( ft17_metadata[ i ])) && ( std::get< 1 >( lot_meta ) == std::get< 1 >( ft17_metadata[ i ]))){
	...
	if( exists ){
	  ...
	  if( QDateTime::fromString( QString::fromStdString( std::get< 2 >( lot_meta ).first.at( "start_datetime" )), "dd.MM.yyyy HH:mm:ss" ).secsTo( QDateTime::fromString( QString::fromStdString( std::get< 2 >( ft17_metadata[ pos ]).first.at( "start_datetime" )), "dd.MM.yyyy HH:mm:ss" )) <= 0 ){

		ft17_metadata[ pos ] = lot_meta;

	  }
	  
	} else ft17_metadata.push_back( lot_meta );
  }

  this->ft17_metadata = ft17_metadata;
}

std::vector< std::tuple< int, std::string, std::pair< std::map< std::string, std::string >, std::map< int, std::map< std::string, std::string >>>, std::string, std::string >> * ft17_parser::get_metadata()
{
  return &this->ft17_metadata;
}

void ft17_parser::print_log( std::vector< std::tuple< int, std::string, std::pair< std::map< std::string, std::string >, std::map< int, std::map<std::string, std::string >>>, std::string, std::string >> & ft17_metadata )
{
  for( unsigned int i = 0; i < ft17_metadata.size(); i ++ ){

	std::printf( "Lot#%i, Job Name : %s, Result : %s, Reason : %s, Tests count : %lu\r\n Header : {"
				 "\r\n\tprog_name : %s,\r\n\tjob_name : %s,\r\n\tpart_name : %s\r\n\topen_datetime : %s,\r\n\tclose_datetime : %s,"
				 "\r\n\trun_number : %s,\r\n\tstart_datetime : %s,\r\n\tstop_datetime : %s,\r\n\toperator_name : %s,\r\n\tlot : %s,"
				 "\r\n\tsite : %s,\r\n\tresult : %s,\r\n\tcategory : %s\r\n}\r\n",
				 std::get< 0 >( ft17_metadata[ i ]),
				 std::get< 1 >( ft17_metadata[ i ]).c_str(),
				 std::get< 3 >( ft17_metadata[ i ]).c_str(),
				 std::get< 4 >( ft17_metadata[ i ]).c_str(),
				 std::get< 2 >( ft17_metadata[ i ]).second.size(),
				 ...
				 std::get< 2 >( ft17_metadata[ i ]).first.at( "lot" ).c_str(),
				 std::get< 2 >( ft17_metadata[ i ]).first.at( "site" ).c_str(),
	...
	for( std::map< int, std::map< std::string, std::string >>::iterator it = std::get< 2 >( ft17_metadata[ i ]).second.begin();
	  ...
	  std::printf( "Test#%i : {\r\n\ttest_time : %s,\r\n\ttest_name_number : %s,\r\n\ttest_name : %s,\r\n\ttest_result : %s,\r\n\ttest_pattern : %s,\r\n\ttest_fails : %s"
				   ...
				   ",\r\n\ttest_force : %s,\r\n\ttest_force_unit : %s,\r\n\ttest_channel : %s,\r\n\t'test_key : %s }\r\n",
				   it->first,
				   it->second.at( "test_time" ).c_str(),
				   it->second.at( "test_name_number" ).c_str(),
				   it->second.at( "test_name" ).c_str(),
				   it->second.at( "test_result" ).c_str(),
				   it->second.at( "test_pattern" ).c_str(),
				   it->second.at( "test_fails" ).c_str(),
				   it->second.at( "test_pin" ).c_str(),
				   it->second.at( "test_measured" ).c_str(),
				   it->second.at( "test_minumum" ).c_str(),
				   it->second.at( "test_maximum" ).c_str(),
				   it->second.at( "test_measurement_unit" ).c_str(),
				   it->second.at( "test_site" ).c_str(),
				   it->second.at( "test_force" ).c_str(),
				   it->second.at( "test_force_unit" ).c_str(),
				   it->second.at( "test_channel" ).c_str(),
...
void ft17_parser::print_report( std::vector< std::tuple< int, std::string, std::pair< std::map< std::string, std::string >, std::map< int, std::map<std::string, std::string >>>, std::string, std::string >> & ft17_metadata )
{
  ...
  for( unsigned int i = 0; i < lots_count; i ++ ) if( std::get< 3 >( ft17_metadata[ i ]) == "FAIL" ){
...
std::vector< std::string > ft17_parser::ft17_get_blocks( std::string & raw_data )
{
  ...
  QRegularExpression header_regexp( "Программа: [\\p{L} \\p{P}\\p{N}\\p{So}\\p{Sm}]*[\u000d\u000a]+Запуск №[\\p{L} \\p{P}\\p{N}\\p{So}\\p{Sm}]*[\u000d\u000a]+Уст-во №[\\p{L} \\p{P}\\p{N}\\p{So}\\p{Sm}]*", QRegularExpression::UseUnicodePropertiesOption );
...
std::string ft17_parser::ft17_get_header( std::string & block )
{
  ...
  std::string result;
  QRegularExpression header_regexp( "Программа:[\\p{L}\\p{Z}\\p{P}\\p{N}\\p{So}\\p{Sm}]*[\u000d\u000a]+Запуск[\\p{L}\\p{Z}\\p{P}\\p{N}\\p{So}\\p{Sm}]*[\u000d\u000a]+Уст-во[\\p{L}\\p{Z}\\p{P}\\p{N}\\p{So}\\p{Sm}]*", QRegularExpression::UseUnicodePropertiesOption );
...
std::string ft17_parser::ft17_parse_prog_name( std::string & header )
{
  ...
  std::function< std::string( std::string &, QRegularExpression & )> parse_substr = []( std::string & raw_data, QRegularExpression & re ) -> std::string {
	QString data = QString::fromStdString( raw_data );
	std::string result;
  ...
  QRegularExpression prog_name_regexp( "Программа: [\\p{Sm}_№\\p{L}\\p{N}]*", QRegularExpression::UseUnicodePropertiesOption );
  QRegularExpression prog_name_substr_regexp( "[ ]+[\\p{Sm}_№\\p{L}\\p{N}]*", QRegularExpression::UseUnicodePropertiesOption );
...
std::string ft17_parser::ft17_parse_job_name( std::string & header )
{
  ...
  QRegularExpression job_name_regexp( "Уст-во: [\\p{Sm}_№\\p{L}\\p{N}]*[+-]*", QRegularExpression::UseUnicodePropertiesOption );
  QRegularExpression job_name_substr_regexp( "[ ]+[\\p{Sm}_№\\p{L}\\p{N}]*[+-]*", QRegularExpression::UseUnicodePropertiesOption );
...
std::string ft17_parser::ft17_parse_part_name( std::string & header )
{
  ...
  QRegularExpression part_name_regexp( "Партия: [\\p{Sm}\\p{L}\\p{N}-]*", QRegularExpression::UseUnicodePropertiesOption );
  QRegularExpression part_name_substr_regexp( "[ ]+[\\p{Sm}\\p{L}\\p{N}\\p{P}]*", QRegularExpression::UseUnicodePropertiesOption );
...
std::string ft17_parser::ft17_parse_open_datetime( std::string & header )
{
  ...
  QRegularExpression open_datetime_regexp( "Открытие: [\\p{N}]+.[\\p{N}]+.[\\p{N}]+ [\\p{N}]+:[\\p{N}]+:[\\p{N}]+", QRegularExpression::UseUnicodePropertiesOption );
  QRegularExpression open_datetime_substr_regexp( "[\\p{N}]+.[\\p{N}]+.[\\p{N}]+ [\\p{N}]+:[\\p{N}]+:[\\p{N}]+", QRegularExpression::UseUnicodePropertiesOption );
...
std::string ft17_parser::ft17_parse_close_datetime( std::string & header )
{
  ...
  QRegularExpression close_datetime_regexp( "Закрытие: [\\p{N}]+.[\\p{N}]+.[\\p{N}]+ [\\p{N}]+:[\\p{N}]+:[\\p{N}]+", QRegularExpression::UseUnicodePropertiesOption );
  QRegularExpression close_datetime_substr_regexp( "[\\p{N}]+.[\\p{N}]+.[\\p{N}]+ [\\p{N}]+:[\\p{N}]+:[\\p{N}]+", QRegularExpression::UseUnicodePropertiesOption );
...
std::string ft17_parser::ft17_parse_start_datetime( std::string & header )
{
  ...
  QRegularExpression start_datetime_regexp( "Старт: [\\p{N}]+.[\\p{N}]+.[\\p{N}]+ [\\p{N}]+:[\\p{N}]+:[\\p{N}]+", QRegularExpression::UseUnicodePropertiesOption );
  QRegularExpression start_datetime_substr_regexp( "[\\p{N}]+.[\\p{N}]+.[\\p{N}]+ [\\p{N}]+:[\\p{N}]+:[\\p{N}]+", QRegularExpression::UseUnicodePropertiesOption );
...
std::string ft17_parser::ft17_parse_stop_datetime( std::string & header )
{
  ...
  QRegularExpression stop_datetime_regexp( "Стоп: [\\p{N}]+.[\\p{N}]+.[\\p{N}]+ [\\p{N}]+:[\\p{N}]+:[\\p{N}]+", QRegularExpression::UseUnicodePropertiesOption );
  QRegularExpression stop_datetime_substr_regexp( "[\\p{N}]+.[\\p{N}]+.[\\p{N}]+ [\\p{N}]+:[\\p{N}]+:[\\p{N}]+", QRegularExpression::UseUnicodePropertiesOption );
...
std::string ft17_parser::ft17_parse_operator( std::string & header )
{
  ...
  std::function< std::string( std::string &, QRegularExpression & )> parse_substr = []( std::string & raw_data, QRegularExpression & re ) -> std::string {
	...
	for( std::string str; ss >> str; ) result = str;
	
	return result;
  };
  
  QRegularExpression operator_regexp( "Пользователь: [\\p{L}\\p{N}\\p{P}]*", QRegularExpression::UseUnicodePropertiesOption );
...
{
  std::function< std::string( std::string &, QRegularExpression &, QRegularExpression & )> get_match = []( std::string & raw_data, QRegularExpression & pattern, QRegularExpression & sub_pattern ) -> std::string {
	std::string result;
	QString data = QString::fromStdString( raw_data );
	QRegularExpressionMatch pattern_match = pattern.match( data );

	if( pattern_match.hasMatch() ) result = pattern_match.captured( 0 ).toStdString();
	...
	if( sub_pattern_match.hasMatch() ) result = sub_pattern_match.captured( 0 ).toStdString();
  ...
  QRegularExpression test_regexp( "([ ]*[\\p{N}]+\\.[ ]+[\\p{N}]+:[\\p{N}]+:[\\p{N}]+\\.[\\p{N}]+[ ]+Тест: [\\p{N}]+\\.[ ]+[\\p{P}\\p{L}\\p{N}]+[ ]+Статус: [\\p{L}]+[ ]+Конт: [\\p{L}\\p{N}\\p{P}]+[ ]+Измер: [\\p{N}\\p{P}\\p{Sm}]+ [\\p{L}]+[ ]+Мин.: [\\p{N}\\p{P}\\p{Sm}]+ [\\p{L}]+[ ]+Макс: [\\p{N}\\p{P}\\p{Sm}]+ [\\p{L}]+[ ]+Сайт:[\\p{N}]+; [\\p{Lu}]=[\\p{N}\\p{P}\\p{Sm}]+ [\\p{L}]+; Кан:[\\p{N}]\\.[\\p{N}]\\.[\\p{N}]; к\\.[\\p{N}]+;)|"
								  "([ ]*[\\p{N}]+\\.[ ]+[\\p{N}]+:[\\p{N}]+:[\\p{N}]+\\.[\\p{N}]+[ ]+Тест: [\\p{N}]+\\.[ ]+[\\p{P}\\p{L}\\p{N}]+[ ]+Статус: [\\p{L}]+[ ]+Патн: [\\p{L}\\p{N}\\p{P}\\p{Z}\\p{Sm}\\p{So}]+)"
								  );
  QRegularExpressionMatchIterator test_regexp_it = test_regexp.globalMatch( data );

  std::string tests_header( "test_number test_time test_name_number test_name test_result test_pattern test_fails test_pin test_measured test_minumum test_maximum test_measurement_unit test_site test_force test_force_unit test_channel test_key" );
  std::map< std::string, std::tuple< QRegularExpression, QRegularExpression >> test_regex_map = {
	{ "test_time", { QRegularExpression( "[ ]+[\\p{N}]+:[\\p{N}]+:[\\p{N}]+\\.[\\p{N}]+" ), QRegularExpression( "[\\p{N}]+:[\\p{N}]+:[\\p{N}]+\\.[\\p{N}]+" )}},
	{ "test_name_number", { QRegularExpression( "Тест: [\\p{N}]+" ), QRegularExpression( "[\\p{N}]+" )}},
	{ "test_name", { QRegularExpression( "Тест: [\\p{N}]+\\.[ ]+[\\p{P}\\p{L}\\p{N}]+" ), QRegularExpression( "[\\p{P}\\p{L}\\p{N}]+" )}},
	{ "test_result", { QRegularExpression( "Статус: [\\p{L}]+" ), QRegularExpression( "[\\p{L}]+" )}},
	{ "test_pattern", { QRegularExpression( "Патн: [\\p{N}\\p{L}\\p{P}_]+" ), QRegularExpression( "[\\p{N}\\p{L}\\p{P}_]+" )}},
	...
	{ "test_measured", { QRegularExpression( "Измер: [\\p{N}\\p{P}\\p{Sm}]+" ), QRegularExpression( "[\\p{N}\\p{P}\\p{Sm}]+" )}},
	{ "test_minumum", { QRegularExpression( "Мин.: [\\p{N}\\p{P}\\p{Sm}]+" ), QRegularExpression( "[\\p{N}\\p{P}\\p{Sm}]+" )}},
	{ "test_maximum", { QRegularExpression( "Макс: [\\p{N}\\p{P}\\p{Sm}]+" ), QRegularExpression( "[\\p{N}\\p{P}\\p{Sm}]+" )}},
	{ "test_measurement_unit", { QRegularExpression( "Измер: [\\p{N}\\p{P}\\p{Sm}]+ [\\p{L}]+" ), QRegularExpression( "[\\p{L}]+" )}},
	...
	{ "test_force_unit", { QRegularExpression( "[\\p{Lu}]=[\\p{N}\\p{P}\\p{Sm}]+ [\\p{L}]+" ), QRegularExpression( "[\\p{L}]+" )}},
	{ "test_channel", { QRegularExpression( "Кан:[\\p{N}]\\.[\\p{N}]\\.[\\p{N}]" ), QRegularExpression( "[\\p{N}]\\.[\\p{N}]\\.[\\p{N}]" )}},
  ...
  for( std::string test : tests ){
	...
	if( test_number_regexp_match.hasMatch() ) test_number = test_number_regexp_match.captured( 0 ).toStdString();
	
	for( unsigned int i = 1; i < tests_header_list.size(); i ++ )
	  test_meta.insert( std::make_pair( tests_header_list[ i ].toStdString(), get_match( test, std::get< 0 >( test_regex_map.at( tests_header_list[ i ].toStdString() )), std::get< 1 >( test_regex_map.at( tests_header_list[ i ].toStdString() )))));

	for( unsigned int i = 0; i < test_meta.at( "test_measured" ).length(); i ++ ) if( test_meta.at( "test_measured" )[ i ] == ',' ) test_meta.at( "test_measured" )[ i ] = '.';
...
std::tuple< std::string, std::string > ft17_parser::ft17_check_tests( std::map< int, std::map< std::string, std::string >> & tests_meta )
{
  std::tuple< std::string, std::string > result;

  for( std::map< int, std::map< std::string, std::string >>::iterator it = tests_meta.begin(); it != tests_meta.end(); it ++ ){

	if( it->second.at( "test_result" ) == "Годен" ) result = { "PASS", "OK" };
