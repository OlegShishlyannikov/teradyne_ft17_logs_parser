...
teradyne_parser::teradyne_parser(std::string &teradyne_raw_data) {
  ...
  for (std::string block : teradyne_blocks) {
    ...
    if (flow_id == "ETT1")
      flow_id = "BEFORE_ETT";
    else if (flow_id == "ETT2")
      flow_id = "AFTER_ETT";
    else if (flow_id == "T_MINUS")
      flow_id = "MINUS";
    else if (flow_id == "T_PLUS")
      flow_id = "PLUS";
    ...
    std::tuple<int, std::string,
               std::pair<std::map<std::string, std::string>, std::map<int, std::map<std::string, std::string>>>,
               std::string, std::string>
        lot_meta = {lot, flow_id, std::make_pair(header_metadata, tests_metadata), std::get<0>(tests_result),
    ...
    for (unsigned int i = 0; i < teradyne_metadata.size(); i++) {
      ...
      if ((std::get<0>(lot_meta) == std::get<0>(teradyne_metadata[i])) &&
          (std::get<1>(lot_meta) == std::get<1>(teradyne_metadata[i]))) {
    ...
    if (exists) {
      ...
      if (QDateTime::fromString(QString::fromStdString(std::get<2>(lot_meta).first.at("date_time")),
                                "MM/dd/yyyy HH:mm:ss")
              .secsTo(QDateTime::fromString(
                  QString::fromStdString(std::get<2>(teradyne_metadata[pos]).first.at("date_time")),
      ...
      teradyne_metadata.push_back(lot_meta);
  }
...
std::vector<std::tuple<int, std::string,
                       std::pair<std::map<std::string, std::string>, std::map<int, std::map<std::string, std::string>>>,
                       std::string, std::string>> *
teradyne_parser::get_metadata() {
  return &this->teradyne_metadata;
}

void teradyne_parser::print_report(
    std::vector<
        std::tuple<int, std::string,
                   std::pair<std::map<std::string, std::string>, std::map<int, std::map<std::string, std::string>>>,
                   std::string, std::string>> &teradyne_metadata) {
  ...
  for (unsigned int i = 0; i < lots_count; i++)
    if (std::get<3>(teradyne_metadata[i]) == "FAIL") {
...
void teradyne_parser::print_log(
    std::vector<
        std::tuple<int, std::string,
                   std::pair<std::map<std::string, std::string>, std::map<int, std::map<std::string, std::string>>>,
                   std::string, std::string>> &teradyne_metadata) {
  for (unsigned int i = 0; i < teradyne_metadata.size(); i++) {
    ...
    std::printf(
        "Lot#%i, FlowID : %s, Result : %s, Reason : %s, Tests count : %lu\r\n Header : {"
        "\r\n\tdate_time : %s,\r\n\tprog_name : %s,\r\n\tjob_name : %s\r\n\tlot : %s,\r\n\toperator : "
        "%s,\r\n\ttest_mode : %s,"
        "\r\n\tnode_name : %s,\r\n\tpart_type : %s,\r\n\tchannel_map : %s,\r\n\tenvironment : %s,\r\n\tfamily_id : %s,"
        "\r\n\tfloor_id : %s,\r\n\tflow_id : %s,\r\n\ttest_temp : %s,\r\n\tuser_text : %s\r\n}\r\n",
        std::get<0>(teradyne_metadata[i]), std::get<1>(teradyne_metadata[i]).c_str(),
        std::get<3>(teradyne_metadata[i]).c_str(), std::get<4>(teradyne_metadata[i]).c_str(),
    ...
    for (std::map<int, std::map<std::string, std::string>>::iterator it =
             std::get<2>(teradyne_metadata[i]).second.begin();
         it != std::get<2>(teradyne_metadata[i]).second.end(); it++) {

      std::printf("Test#%i : { Site : %s, Result : %s, Test Name : %s, Pin : %s, Channel : %s, Low : %s, Measured : "
                  "%s, High : %s, Force : %s, Loc : %s }\r\n",
                  it->first, it->second.at("site").c_str(), it->second.at("result").c_str(),
                  it->second.at("test_name").c_str(), it->second.at("test_pin").c_str(),
                  it->second.at("channel").c_str(), it->second.at("test_minimum").c_str(),
                  it->second.at("test_measured").c_str(), it->second.at("test_maximum").c_str(),
                  it->second.at("force").c_str(), it->second.at("loc").c_str());
    }
  }
}

std::tuple<std::string, std::string>
teradyne_parser::teradyne_check_tests(std::map<int, std::map<std::string, std::string>> &tests_meta) {
  std::tuple<std::string, std::string> result;

  for (std::map<int, std::map<std::string, std::string>>::iterator it = tests_meta.begin(); it != tests_meta.end();
       it++) {

    if (it->second.at("result") == "PASS")
      result = {"PASS", "OK"};
...
std::vector<std::string> teradyne_parser::teradyne_get_blocks(std::string &teradyne_raw_data) {
  ...
  QRegularExpression block_regexp("Datalog "
                                  "report[[:space:]]+[[:digit:]]+/[[:digit:]]+/"
                                  "[[:digit:]]+[[:space:]]+[[:digit:]]+:[[:digit:]]+:[[:digit:]]+[[:space:]]+"
...
std::string teradyne_parser::teradyne_get_header(std::string &teradyne_block) {
  ...
  std::string result;
  QRegularExpression header_regexp("Datalog "
                                   "report[[:space:]]+[[:digit:]]+/[[:digit:]]+/"
                                   "[[:digit:]]+[[:space:]]+[[:digit:]]+:[[:digit:]]+:[[:digit:]]+[[:space:]]+"
                                   "Prog Name:[[:space:]]*[[:graph:]]*[[:space:]]+"
                                   "Job Name:[[:space:]]*[[:graph:]]*[[:space:]]+"
...
std::string teradyne_parser::teradyne_get_body(std::string &teradyne_block) {
  QString data = QString::fromStdString(teradyne_block);
  std::string result;
  QRegularExpression body_regexp("Number[[:space:]]+Site[[:space:]]+Result[[:space:]]+Test "
                                 "Name[[:space:]]+Pin[[:space:]]+Channel[[:space:]]+Low[[:space:]]+Measured[[:space:]]+"
                                 "High[[:space:]]+Force[[:space:]]+Loc[[:space:]]+[^=]*[=]+");
...
std::string teradyne_parser::teradyne_parse_datetime(std::string &teradyne_header) {
  std::function<std::string(std::string &, std::regex &)> parse_substring = [](std::string &to_parse,
                                                                               std::regex &re) -> std::string {
...
std::map<int, std::map<std::string, std::string>> teradyne_parser::teradyne_get_tests(std::string &teradyne_body) {
  ...
  QRegularExpression test_regexp(
      "[[:graph:]]+[ ]{2,}[[:graph:]]+[ ]{2,}[[:word:]]{4}[ ]{2,}[[:graph:]]+[ ]{2,}[[:graph:]]+[ ]{2,}[[:graph:]]+[ "
      "]{2,}[[:graph:]]+[ ]?[[:word:]]+[ ]{2,}[[:graph:]]+[ ]?[[:word:]]+[ ]{2,}[[:graph:]]+[ ]?[[:word:]]+[ "
      "]{2,}[[:graph:]]+[ ]?[[:word:]]+[ ]{2,}[[:digit:]]");
  ...
  for (std::string test : tests) {
    ...
    std::string test_header(
        "number  site  result  test_name  test_pin  channel  test_minimum  test_measured  test_maximum  force  loc");
